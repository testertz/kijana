...
const partners: Partner[] = [
  { id: 1, name: 'Morogoro Farms', logo: 'https://placehold.co/150x60?text=Morogoro' },
  { id: 2, name: 'Dar Fresh Co.', logo: 'https://placehold.co/150x60?text=DarFresh' },
  { id: 3, name: 'Tanzania Tea Co.', logo: 'https://placehold.co/150x60?text=TzTea' },
  { id: 4, name: 'Zanzibar Spices', logo: 'https://placehold.co/150x60?text=ZzSpices' },
  { id: 5, name: 'Arusha Organics', logo: 'https://placehold.co/150x60?text=ArushaBio' },
  { id: 6, name: 'Kilimanjaro Goods', logo: 'https://placehold.co/150x60?text=KiliGoods' },
];
  ...
  return (
    <div className="pt-16 sm:pt-20">
      ...
      <section className="bg-cream-100 py-16 sm:py-20">
        <div className="container-px">
          <div className="mx-auto max-w-2xl text-center">
            <p className="eyebrow">Washirika Wetu</p>
            <h2 className="section-title mt-2">Our Partners</h2>
            <p className="mt-4 text-earth-700">We are proud to work alongside organizations that share our commitment to natural, healthy and authentic Tanzanian food.</p>
          </div>
          <div className="mt-10 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6">
            {partners.map((p) => (
              <div
                key={p.id}
                className="group flex flex-col items-center justify-center gap-3 rounded-2xl border border-earth-200/70 bg-white px-4 py-6 transition-all hover:-translate-y-1 hover:shadow-card"
              >
                <img
                  src={p.logo}
                  alt={p.name}
                  loading="lazy"
                  className="h-12 w-full max-w-[150px] object-contain opacity-60 grayscale transition-all duration-300 group-hover:opacity-100 group-hover:grayscale-0 group-hover:scale-105"
                />
                <p className="text-xs font-medium text-earth-600 transition-colors group-hover:text-forest-900">{p.name}</p>
              </div>
            ))}
          </div>
        </div>
